# This is an auto-generated Django model module.
# You'll have to do the following manually to clean this up:
#   * Rearrange models' order
#   * Make sure each model has one field with primary_key=True
#   * Make sure each ForeignKey has `on_delete` set to the desired behavior.
#   * Remove `managed = False` lines if you wish to allow Django to create, modify, and delete the table
# Feel free to rename the models, but don't rename db_table values or field names.
from django.db import models


class ChContactTbl(models.Model):
    contact_id = models.BigIntegerField(primary_key=True)
    contact_email = models.CharField(max_length=128)
    contact_url = models.TextField()
    crawl_status = models.IntegerField(default=0)
    post_timestamp = models.DateTimeField(auto_now_add=True)
    mail_error = models.CharField(max_length=500, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'ch_contact_tbl'

class ChUnsubscriberTbl(models.Model):
    id = models.BigIntegerField(primary_key=True)
    email = models.CharField(max_length=128)

    class Meta:
        managed = False
        db_table = 'ch_unsubscriber_tbl'
